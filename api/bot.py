...
from flask_cors import CORS # CORS এর জন্য ইম্পোর্ট
    ...
    if update.message and update.message.text:
        ...
        if text.startswith('/start'):
            ...
            if not existing_user.data:
                ...
                supabase.table('users').insert({'user_id': user.id, 'first_name': user.first_name, 'username': user.username or '', 'referral_code': generate_referral_code(), 'rix_balance': initial_balance, 'referred_by': referrer_id, 'daily_tasks_completed': 0, 'last_task_reset': datetime.now(timezone.utc).strftime('%Y-%m-%d')}).execute()
    ...
    try:
        ...
        if response.data:
            ...
            if insert_response.data and len(insert_response.data) > 0:
                ...
                print(f"CRITICAL: Failed to insert new user {user_id}. Supabase response: {insert_response}")
                return jsonify({"error": "Could not create user profile. Please try again."}), 500
    except Exception as e:
        print(f"CRITICAL Error in get_user_data: {e}"); return jsonify({"error": f"Internal server error: {str(e)}"}), 500
...
def complete_task_api():
    try:
        ...
        print(f"Error in complete_task: {e}"); return jsonify({"error": f"Internal server error: {str(e)}"}), 500
...
def claim_mining_api():
    try:
        ...
        print(f"Error claiming mining reward: {e}"); return jsonify({"error": f"Internal server error: {str(e)}"}), 500
