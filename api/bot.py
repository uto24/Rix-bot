...
async def handle_update(update_data):
    ...
    if update.message and update.message.web_app_data:
            ...
            if action == 'get_user_data':
                ...
                if user_db_data:
                    ...
                    if not last_claim_str: can_claim = True
                        ...
                        if now_utc >= next_claim_time: can_claim = True
                        else: next_claim_in_seconds = (next_claim_time - now_utc).total_seconds()
        ...
        except Exception as e: print(f"Error processing web_app_data: {e}")
    ...
    # ২. /start কমান্ড হ্যান্ডেল করা
        ...
        if len(command_parts) > 1:
            ...
            referrer_data = supabase.table('users').select('user_id', 'first_name').eq('referral_code', referral_code).single().execute()
        ...
        else: welcome_message = f"ফিরে আসার জন্য ধন্যবাদ, {user.first_name}!"
    ...
    # ৩. ইনলাইন বাটন ক্লিক হ্যান্ডেল করা
        ...
        if query.data == "refer_friend":
            ...
            keyboard = [[InlineKeyboardButton("📤 বন্ধুদের শেয়ার করুন", url=share_url)],[InlineKeyboardButton("⬅️ মেনুতে ফিরুন", callback_data="back_to_menu")]]
...
def webhook_handler():
    try: asyncio.run(handle_update(request.json))
    except Exception as e: print(f"ERROR in webhook_handler: {e}")
    return 'ok'
...
def set_webhook():
    try:
        ...
        if is_set: return "Webhook has been set successfully!"
        else: return "Failed to set webhook. Please check your TELEGRAM_TOKEN."
    except Exception as e:
